...
def process_stock_data(ticker, is_reit=False):
    ...
    # FIX: Use empty string instead of None for missing sector/category
    sector = data.get('sector', '')
    if not sector or pd.isna(sector):
        sector = 'Real Estate' if is_reit else ''
    
    category = data.get('category', '')
    if not category or pd.isna(category):
        category = 'ETF' if not is_reit else ''
    ...
    result = {
        ...
        'sector': str(sector) if sector else ('Real Estate' if is_reit else ''),
        'category': str(category) if category else ('ETF' if not is_reit else '')
    }
